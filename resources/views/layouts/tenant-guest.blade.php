...
<body
    class="h-full antialiased plan-{{ $theme['slug'] ?? 'basic' }}"
    x-data="{
...
<div class="min-h-screen flex">
    ...
    <div class="hidden lg:flex lg:w-[52%] xl:w-[55%] relative flex-col justify-between bg-gradient-to-br {{ $theme['heroGradient'] }} overflow-hidden">
        ...
        <div class="absolute -top-32 -left-32 w-96 h-96 rounded-full opacity-[0.08] blur-3xl bg-gradient-to-br from-indigo-600 to-violet-700"></div>
        <div class="absolute bottom-1/4 right-1/4 w-64 h-64 rounded-full opacity-[0.05] blur-3xl bg-cyan-500"></div>

        <div class="relative z-10 flex flex-col justify-center flex-1 px-16 py-20">
            <div class="flex items-center gap-4 mb-10">
                <div class="w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg ring-1 ring-white/10 {{ $theme['brandIcon'] }}">
            ...
            @if($plan)
            <div class="mb-8">
                <span class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold border {{ $theme['badge'] }}">
                ...
                @foreach(array_slice($badges, 0, 5) as $line)
                    <li class="flex items-center gap-3 text-slate-300 text-sm">
                        <div class="w-6 h-6 rounded-lg bg-white/10 flex items-center justify-center shrink-0">
                            <svg class="w-3.5 h-3.5 {{ $theme['heroAccent'] }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <div class="flex flex-1 flex-col justify-center relative bg-white dark:bg-slate-950 px-6 py-12 sm:px-12 lg:px-16 xl:px-24 transition-colors duration-300">
        ...
        <div class="mx-auto w-full max-w-md">
            <div class="lg:hidden flex items-center gap-3 mb-10">
                <div class="w-10 h-10 rounded-xl flex items-center justify-center ring-1 ring-white/10 {{ $theme['brandIcon'] }}">
