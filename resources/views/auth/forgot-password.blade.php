...
<x-central-guest-layout>
    ...
    <form method="POST" action="{{ route('password.email') }}" class="space-y-5">
        ...
        <button type="submit" class="w-full py-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold shadow-lg shadow-indigo-600/30">Email reset link</button>
    </form>
    <p class="mt-6 text-center text-sm text-slate-500 dark:text-slate-400"><a href="{{ route('login') }}" class="text-indigo-600 dark:text-indigo-400 font-medium">Back to sign in</a></p>
</x-central-guest-layout>
@else
